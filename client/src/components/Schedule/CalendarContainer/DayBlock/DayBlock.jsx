	...
	useEffect(() => {
		if (teacher_preferences) {
			...
			const start_time = moment.utc(
				`${format_date} ${teacher_preferences.start}`
			);
	...
	return (
		<div className={style.track}>
			{_.range(start_hour, end_hour).map((value, index) => {
				...
				const formatted_start = preferences_start.format(
					ConstantsCalendarContainer.TIME_FORMAT
				);
				const formatted_time = time_obj.format(
					ConstantsCalendarContainer.TIME_FORMAT
				);
