...
async function AddBlockedEvent(date, hour) {
	try {
		const add_blocked_event = await ApiService.AddNewResourceRequest(
			`event/blocked`,
			{ date: date.utc().format(ConstantsHourBlock.DATE_FORMAT), hour }
		);
...
async function AddEvent(date, hour, teacher_id, subject_id) {
	try {
		const add_blocked_event = await ApiService.AddNewResourceRequest(`event`, {
			date: date.utc().format(ConstantsHourBlock.DATE_FORMAT),
			hour,
			teacher_id,
			subject_id,
		});
...
async function DeleteEvent(event_id) {
	try {
		const delete_event = await ApiService.DeleteResourceRequest(
			`event/${event_id}`
		);
