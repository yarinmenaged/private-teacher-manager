...
export default class serverConnection {
  ...
  static async editPrice(id, newPrice) {
    return await ApiService.PutResourceRequest(`users/price/${id}`, {
      newPrice,
    });
    //TODO: error
  }
  ...
  static async addSubject(id, subject) {
    return await ApiService.AddNewResourceRequest(`subjects/${id}`, {
      subject,
    });
    //TODO: error
  }
  ...
  static async removeSubject(id, subject) {
    return await ApiService.AddNewResourceRequest(`subjects/remove/${id}`, {
      subject,
    });
    //TODO: error
  }
  ...
  static async getUserInfo(email, password) {
    try {
      return await ApiService.AddNewResourceRequest(`users/login`, {
        email,
        password,
      });
    } catch (err) {
